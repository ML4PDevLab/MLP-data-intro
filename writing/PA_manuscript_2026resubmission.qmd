...
Finally, since *ML4P* is not an event count, we ask how it relates to a well-recognized event count dataset. Here we assess *ML4P's* capacity to measure the importance of protests in a way that is distinctive. We sum ACLED protest events by month and compare them our monthly measure of protest salience (Appendix G). The two correlate only moderately within countries (median 0.39), and only about a third of the country-months either flags as unusually high are common to the two. The divergence is systematic. *ML4P* rises without a rise in ACLED counts when a single large mobilization dominates the news. For example, Pakistan’s 2014 Islamabad sit-in falls in the 7th percentile of its ACLED distribution but the 100th of *ML4P's*.[^10] Conversely, ACLED counts rise without a rise in *ML4P* when many small, dispersed gatherings occur, or when another story dominates the agenda. Because ACLED counts also trend upward with its expanding coverage while our normalized share does not, the two answer different questions: how much protest occurred, versus how much it mattered for civic space.[^11] We find no evidence of false positives in *ML4P's* protest data when we compare it with ACLED. 

[^10]: In some cases, deviations emerge from ACLED's exclusion of strikes and stay-aways from its definition of protest and/or its coding of any protest that turns violent as a riot. See Appendix.
[^11]: ACLED provides data on the size of protests, but it is missing for the vast majority of protests in the 7071 country-months that overlap with *ML4P*.   
