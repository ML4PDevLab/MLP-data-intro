...
## Section 1: Civic Space Coverage Intensity
...
## Section 2: Dominant Civic Events by Country
...
## Section 1: State of Emergency Detection by Region
...
# Appendix G: Comparison of ML4P and ACLED Protest DAta

To compared ACLED and ML4P protest data, we first sum ACLED counts by month. This produces an overlap of 7,219 country-months between the two sources. We exclude analysis of ACLED's crowd-size tag because it is empty or not reported for 83% of protests and not specific for an additional 10%. For each country we compute the correlation between ACLED monthly counts and the ML4P measure across its months. The median correlation between ACLED protest counts and the ML4P's protest salience measure is 0.39. Taking the top decile of each series within each country, the two overlap on only 31% of country-months. In countries with full coverage from 2012 through the end of the series, ACLED mean monthly protest counts rise from 29.2 to 77.0, which partially reflects a well understood expansion of ACLED coverage over time; ML4P's protest share is more steady, falling slightly in aggregate from 1.7% to 1.6% over the time series.  

To better understand the characteristics of the two series, we focus on the most divergent months between ML4P and ACLED, i.e. the largest monthly surges in ML4P protest salience when ACLED counts do *not* increase and vice-verse. We flag surges as any month exceeding twice its 25-month rolling median while falling in the country's top decile. We did desk research on the 25 biggest gaps across distinct country-months. Table 6 presents the 13 cases in which the largest ML4P surges are not associated with surges in ACLED, including the total number of ACLED protests, protest salience (i.e. protest attention share) from ML4P, and how those compare with their within-country distributions.

![](descriptive_maps/figures/highML4P.png)

These cases reflect three classes of difference between the two sources. The first involve a small number of very large, politically momentous mobilizations: Turkey in 2017, Pakistan in 2014, Liberia in 2019, and Philippines in 2016 are all examples. Each of these are seminal moments in the recent protest history of these countries. A second class reflects ACLED's definition and coding of riots and violence against civilian. If a peaceful protest turns violent on the same day ACLED records it as a Riot event, not a Protest event. This includes the cases of Sri Lanka, July 2022; South Africa, July 2021; Senegal, March 2021; Malawi, August 2019; and Zimbabwe, August 2019.  Third, is a set of cases where withdrawals or strikes serve as the forms of protest, which ACLED does not record. These include Cameroon, December 2016; Kenya, December 2016; Tunisia, June 2022; Ghana, October 2024 (though in this last case there were also 3 days of mass demonstrations in Accra).

Table 7 provides the inverse set of cases, i.e. those where a surge in the number of ACLED protests is not matched with an increase in ML4P's protest measure. Analysis of these cases suggests three classes of cases. First are dispersed, routine, or localized mobilization. Nepal in July 2022 records 462 protest events against a flat ML4P score. In July 2022 Nepal held elections at every level of government that we met with decentralized protests in some localities (note that ACLED had also recently completed a supplemental coding project covering Nepal from 2018 with local partners). Tunisia's October 2020 regional economic protests, Bangladesh's May 2021 wave of small labour actions and similar protests in  Guatemala in January 2022 and Cambodia in August 2016 are of the same type. These cases underscore that ML4P places less weight on small local protests relative to ACLED. Second, the wave of July 2016 protests recorded in ACLED are reflected in a substantial number of articles covering protests in the ML4P corpus (126), but the salience of those protests are crowded out by the failed coup on July 15. ML4P captures that singular event with a large spike in its "irregular transitions" civic variable. From the point of view of civic space, the coup is the defining civic event of the month even if there were many protests. Third and finally, the Ethiopia in May 2018 points to a legitimate weakness in the ML4P approach, namely that it can be sensitive to overall corpus size and limitations of press freedom. The manuscript addresses these in its 'Limitations' section. 

![](descriptive_maps/figures/lowML4P.png)

We also used this exercise as a further validation check on the ML4P data by reviewing the 2825 individual protest articles across the 25 country-months in the two tables above. We identified 100 false positives constituting only 3.5\% of articles, and more than half of those (53) were a result of geoparsing failures. For instance, ML4P records a July 2022 article on protest in Nepal. Though published by a Nepali source (onlinekhabar.com), the article actually reports on a protest in Congo against the UN mission in that country which had endangered Nepali peacekeepers. Likewise, the Turkish press reports in several cases on Turkish protests for or against the government in foreign capitals during the July 2016 coup attempt.  

In sum, ACLED event counts are sensitive to the number, dispersion, and duration of discrete gatherings. ML4P protest is sensitive to the political weight a national public and its press assigned to the protests. These are different quantities, and they disagree systematically rather than randomly. 

# Appendix H: Forecasting Data and Models

This appendix provides additional information on the two forecasting excercises presented in Section 5 of the manuscript. For additional technical details, refer to @hlta2025report.

## Section 1: Forecasting Conflict Onset

We develop two models using a LightGBM gradient-boosted tree classifier. The target variable is conflict onset from the UCDP/PRIO Armed Conflict Dataset (v24.1), defined as at least 25 battle-related deaths in a country-month with no ongoing conflict in the prior year. Because conflict onsets account for roughly 1\% of country-month observations, both models incorporate class weighting. The *ML4P* model's feature set consists of monthly *ML4P* civic salience measures stacked over a 12-month lag window to capture both proximate and more distal shifts in civic activity. These are supplemented by a Bayesian country-level baseline conflict risk encoding that pools each country's historical conflict rate toward the global mean, a months-since-last-conflict counter, an indicator of whether a conflict occurred within the past 12 months conflict, and country-specific z-scored monthly economic indicators from Trading Economics. The second model includes data from ACLED (v2024-01) instead of *ML4P*: six event types (battles, explosions/remote violence, protests, riots, strategic developments, and violence against civilians) aggregated to the country-month level. Everything else, including the lag structure, model architecture, and tuning procedure, remains unchanged.

Both models are evaluated at 3- and 6-month forecast horizons under two schemes. The out-of-sample (OOS) setup trains on January 2012–December 2022 and tests on the full year 2023. Temporal cross-validation (TCV) uses rolling-origin folds in which the training window expands sequentially to each time cut and the model is evaluated on the immediately subsequent period, with folds synchronized to *ML4P's* quarterly data release cycle to ensure that only information available at prediction time enters training. Performance is assessed via ROC-AUC, AUC-PR, and Brier Score; AUC-PR results are benchmarked against a dummy classifier that assigns probabilities based on the baseline rate of conflict onset in the training data. @tbl-forecast-onset-performance presents peformance metrics for our forecasting models.

```{r}
#| label: tbl-forecast-onset-performance
#| tbl-cap: "Conflic onset forecast performance across evaluation schemes and horizons"
#| echo: false
#| warning: false

tribble(
  ~`Evaluation Method`, ~`Horizon (mo.)`, ~`ROC AUC`, ~`AUC PR`, ~`Brier Score`,
  "OOS (MLP)",   3, 0.80, 0.04, 0.29,
  "TCV (MLP)",   3, 0.81, 0.05, 0.26,
  "TCV (ACLED)", 3, 0.78, 0.04, 0.27,
  "OOS (MLP)",   6, 0.87, 0.06, 0.01,
  "TCV (MLP)",   6, 0.73, 0.03, 0.26,
  "TCV (ACLED)", 6, 0.72, 0.03, 0.21
) |>
  kable(
    format   = "latex",
    booktabs = TRUE,
  ...
  footnote(
    general_title = "Notes:",
    general = paste(
      "OOS = out-of-sample (single holdout period);",
      "TCV = temporal cross-validation (rolling-origin folds that preserve time order).",
...
![Top-ten average feature importances for the *ML4P* (left) and ACLED (right) models at the 3-month.](descriptive_maps/figures/FSE_forecasting_ML4PvsACLED_3m.png)


![Top-ten average feature importances for the *ML4P* (left) and ACLED (right) models at the 6-month.](descriptive_maps/figures/FSE_forecasting_ML4PvsACLED_6m.png)


## Section 2: Forecasting DoS' HLTAs

The HLTA forecasting model builds on the same LightGBM framework and *ML4P* covariate structure as the conflict onset models but introduces several modifications tailored to the travel advisory target. The target variable is the onset of a U.S. Department of State high-level travel advisory (Levels 3 or 4 in the post-2018 four-level system, or any advisory in the pre-2018 period), compiled monthly for all 60 countries in the MLP dataset from January 2012 through December 2023. Travel advisory onsets account for only 1.42\% of country-month observations. We construct two onset measures: a fixed indicator (value of 1 for the specific month of onset) and a rolling quarterly window (value of 1 for the month before, the month of, and the month after onset), the latter capturing advisories that may fall slightly outside rigid monthly boundaries. In addition to the *ML4P* civic salience measures and economic z-scores, the model incorporates a Bayesian country-level baseline encoding re-estimated for travel advisory onset rates; a "continued advisory" covariate that flags country-months in which an advisory begun in a prior period remains active, distinguishing persistence from new issuance; and a COVID-19 indicator marking advisories explicitly linked to the pandemic, which produced a sharp structural break in onset counts during 2020–2022.

Unlike the conflict onset forecasting exercise, we do not present a comparison with an ACLED-based forecasting model for HLTAs, since ACLED event data are not expected to capture the broader range of threats---including health crises, natural disasters, and crime---that drive travel advisory issuance.

Table @tbl-forecast-hlta-performance reports forecast performance for the HLTA forecast model across 3- and 6-month horizons and two onset window specifications. The fixed measure takes the value of 1 for the specific month of an advisory onset; the rolling quarterly window (±1 month) additionally captures advisories that may fall slightly outside rigid monthly reporting boundaries. ROC-AUC scores are consistently strong across all specifications (0.87–0.90), indicating reliable discrimination between onset and non-onset country-months. The AUC-PR metric, which is more sensitive to the rarity of onsets (1.42% of country-months), improves substantially under the rolling window specification: AUC-PR rises from 0.26 to 0.54 at the 3-month horizon and from 0.31 to 0.57 at the 6-month horizon, in both cases far exceeding the dummy model baseline (0.01–0.02). The dummy model assigns every observation the unconditional onset probability and thus represents the floor of performance achievable without any predictive signal; the 26- to 57-fold improvement in AUC-PR confirms that the civic salience measures capture genuine early-warning information beyond the base rate. Brier Scores are low across all specifications (0.12–0.14), indicating well-calibrated probability estimates.

```{r}
#| label: tbl-forecast-hlta-performance
#| tbl-cap: "HLTAs' forecast performance across evaluation schemes and horizons"
#| echo: false
#| warning: false

tribble(
  ~`Horizon (mo.)`, ~`ROC AUC`, ~`AUC PR`, ~`Dummy AUC PR`, ~`Brier Score`,
  "3-Month Fixed",   0.87, 0.26, 0.01, 0.14,
  "3-Month Rolling Window (±1 Month)", 0.89, 0.54, 0.02, 0.126,
  "6-Month Fixed", 0.87, 0.31, 0.01, 0.14,
  "6-Month Rolling Window (±1 Month)", 0.90, 0.57, 0.02, 0.13
) |>
  kable(
    format   = "latex",
    booktabs = TRUE,
  ...
  footnote(
    ...
    general = paste(
      "ROC AUC = receiver operating characteristic area;",
      "AUC PR = area under the precision--recall curve;",
      "Dummy AUC PR = AUC-PR of a baseline model that assigns every observation the unconditional onset probability.",
      "Brier = mean squared error of probabilistic forecasts."
    ),
    escape         = FALSE,
    threeparttable = TRUE
...
<!-- Our LightGBM models evaluated via temporal cross-validation achieve a ROC-AUC of 0.87, AUC-PR of 0.26, and Brier score of 0.14 at the 3-month horizon against a dummy AUC-PR of 0.01; counting predictions as correct within ±1 month of the forecast improves performance to ROC-AUC 0.89, AUC-PR 0.54, and Brier 0.12. Performance results for our 6-month model yield a ROC-AUC of 0.87, AUC-PR of 0.31, and a Brier score of 0.14 (dummy AUC-PR = 0.01), while the rolling-window variant (±1 month) increases ROC-AUC to 0.90, AUC-PR to 0.57, and lowers the Brier score to 0.13, again with a dummy AUC-PR of 0.02. -->

![Top 10 Average Feature Importances for 3- and 6-Month Forecasts.](descriptive_maps/figures/FSE_forecasting_hlta.png)
